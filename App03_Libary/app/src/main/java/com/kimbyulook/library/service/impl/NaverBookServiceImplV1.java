package com.kimbyulook.library.service.impl;
...
import com.kimbyulook.library.adapter.BookViewAdapter;
import com.kimbyulook.library.config.NaverAPI;
import com.kimbyulook.library.model.NaverBookDTO;
import com.kimbyulook.library.model.NaverParent;
import com.kimbyulook.library.service.NaverBookService;
import com.kimbyulook.library.service.RetrofitAPIClient;
...
public class NaverBookServiceImplV1 implements NaverBookService {
    ...
    public NaverBookDTO getBooks(String search){
         ...
         *      RetrofitAPIClient의 getApiClient() method 는
         ...
         *      하출한 곳에서는 호출만하고
         *      즉시 , 자신의 다른일을 수행하는 것을 계속한다
         *      응답이 오면 그 응답을 처리할 Callback 을 적절히 처리해 주어야한다
         *      event 핸들링 처리라고도 한다
         *
         *   3.동기 방식
         ...
         *
         */
       Call<NaverParent> retrofitReturn =  RetrofitAPIClient
                .getApiClient()
                .getBook(
                        NaverAPI.CLIENT_ID
                        ,NaverAPI.CLIENT_SECTRET
                        ,search
                        ,1
                        ,10);
        /**
...
//                    도서 리스트를 사용하여 RecyclerView에 데이터를 표현하기위한 Adapter를 생성하기
