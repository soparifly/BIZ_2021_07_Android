package com.kimbyulook.word.dao;
 ...
 * 화면에 실제 보여지는 데이터는 10개단위라고 하자
 *
 *
 * List<DTO> 타입의 method 를 사용하면 100 개 데이터를 모두 읽어서
 * list 에 담아서 메모리에 저장을 한다
 * 이는 메모리에 부담이 되고 디바이스 시스템에 무리를 줄 수도 있다
 *
 * 하지만 LiveData로 감싼 리스트는 안드로이드 OS차원에서 적절히 메모리를 분배하여 리스트를 관리해주는 효과가 있다
 *
 *
 *
